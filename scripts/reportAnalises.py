...
# # Analise folder /workspaces/IC/SSP/output/GATestes
# # python3 ./scripts/reportAnalises.py ./output/GATestes 4
...
def analise_folder():
    folder = "/workspaces/IC/SSP/input/MyInstancesDiferentToolSets"
    ...
    for file in files:
        path = f"{folder}/{file}"
        jobs = ld.loadJobs(path)

        unique_tools = set()

        for job in jobs:
            toolset_id = job['ToolSet']
            tools = toolSets[toolset_id]

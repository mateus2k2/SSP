...
std::tuple<int, int, int, int>  SSP::evaluateReportKTNSMachine(vector<int> s, fstream& solutionReportFile, int machine) {
    vector<bool> magazineL(numberToolsReal, true);
    unsigned int switchs = 0;
    int numberJobsSol = s.size();
    int jL;
    ...
    //iterate of the jobs in s and count the number priority jobs

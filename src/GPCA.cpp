...
// 		for (auto t =
// originalJobs[s[e]].toolSetNormalized.tools.begin(); t !=
// originalJobs[s[e]].toolSetNormalized.tools.end(); ++t){ 			if(last_full <=
// last_seen[*t]){
// 				++pipes_count;
// 				currantSwitchs += 1;
// 				for(unsigned int i = (last_seen[*t]+1); i < e;
// ++i){ 					toolChange = true; 					M[i].push_back(*t); 					if(M[i].size() ==
// capacityMagazine) last_full = i;
// 				}
// 			}
// 			last_seen[*t] = e;
// 		}
...
// 		if( ((inicioJob % DAY) >= unsupervised && (currantSwitchs > 0))
// ||
// //verificar se estou em um periodo sem supervisao e houve troca de ferramenta
// 		    (inicioJob % (planingHorizon*DAY) +
// (originalJobs[s[e]].processingTime) > (planingHorizon*DAY)) ){ //verificar se
// o job excede o horizonte de planejamento unico (iria extender de uma maquina
// para outra)
...
// 		std::set_difference(M[e].begin(), M[e].end(), M[e-1].begin(),
// M[e-1].end(), std::inserter(diff, diff.begin()));
...
// 	int cost = (PROFITYFINISHED * fineshedJobsCount) - (COSTSWITCH *
// (toolSwitches)) - (COSTSWITCHINSTANCE * switchsInstances) - (COSTPRIORITY *
// unfineshedPriorityCount);

// 	return -cost;
// }

// double SSP::evaluateReportGPCA(solSSP solution, string filenameJobs, string
// filenameTools, string solutionReportFileName, int time){ 	vector<vector<int>>
// fineshedIndex; 	vector<vector<int>> fineshedInicio; 	vector<vector<int>>
// fineshedFim; 	vector<int> machines;

// 	// iterate over numberMachines and push vector to each fineshedIndex,
// fineshedInicio, fineshedFim 	for(int i = 0; i < numberMachines; i++){
// 		vector<int> tmp;
...
// 	solutionReportFile << planingHorizon << ";" << unsupervised << ";" <<
// DAY << endl;
...
// originalJobs[s[e]].toolSetNormalized.tools.begin(); t !=
// originalJobs[s[e]].toolSetNormalized.tools.end(); ++t){ 			if(last_full <=
// last_seen[*t]){ 				int tool = reverseSearchMap(*t, ferramentas);
...
// 				currantSwitchs += 1;
// 				for(unsigned int i = (last_seen[*t]+1); i < e;
// ++i){ 					M[i].push_back(*t); 					if(M[i].size() == capacityMagazine) last_full = i;
// 				}
// 			}
// 			last_seen[*t] = e;
// 		}
...
// 		if (toolChange) ++switchsInstances;
// 		toolSwitches += currantSwitchs;
// 		cout << "e: " << e << " currantSwitchs: " << currantSwitchs <<
// endl;
...
// 		// if (!isFirstJobOfMachine) toolSwitches += diff.size();
// 		// if (diff.size() > 0 && !isFirstJobOfMachine)
// ++switchsInstances;
...
// 		int inicioTMP = (fimJob - originalJobs[s[e]].processingTime) %
// (planingHorizon*DAY); 		int fimTMP    = ((fimJob-1) % (planingHorizon*DAY))+1;
...
// 				int realTool = reverseSearchMap(M[countTmp][t],
// ferramentas); 				solutionReportFile << realTool << ",";
// 			}
// 			solutionReportFile << "\n";
// 			countTmp++;
// 		}
